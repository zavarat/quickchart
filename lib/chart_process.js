...
const { DEFAULT_COLORS, DEFAULT_COLOR_WHEEL, ROUND_CHART_TYPES } = require('./charts');
...
async function doRenderChart(width, height, backgroundColor, devicePixelRatio, untrustedChart) {
  ...
  try {
    const buf = await canvasRenderService.renderToBuffer(chart);
...
(async function() {
  ...
  const width = parseInt(argv[2], 10);
  const height = parseInt(argv[3], 10);
  const backgroundColor = argv[4];
  const devicePixelRatio = parseFloat(argv[5]);
  const untrustedChart = argv[6];
  try {
    ...
    // This console log is required! The program's output is read.
    console.log(result);
  } catch (err) {
