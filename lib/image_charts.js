...
function setDataLabels(chl, chartObj) {
  // TODO(ian): This behaves different on bar vs line
  if (!chl) {
    return;
  }
  ...
  chartObj.options.plugins.datalabels = {
    // TODO(ian): Change Datalabel placement to match Google's
    display: true,
    formatter: (val, ctx) => {
      let labelIdx = 0;
...
function setAxes(chxt, chxr, chartObj) {
  ...
  if (chxr) {
    ...
    axesSettings.forEach(axisSetting => {
      ...
      let axis;
      if (axisName === 'x') {
        // TODO(ian): X-axis is not correctly scaled if data values don't
        // extend the full range: chart.js doesn't respect min/max.
...
function setMarkers(chm, chartObj) {
  ...
  chm.split('|').forEach(markerRule => {
    const parts = markerRule.split(',');
    // Ignore marker type - we're only showing data labels for now, and we
...
function toChartJs(query) {
  ...
  // TODO(ian): chf chart fill
  // TODO(ian): chg grid lines
