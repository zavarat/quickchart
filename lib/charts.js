...
const { getProcess } = require('./process_wrapper');
...
function renderChart(width, height, backgroundColor, devicePixelRatio, untrustedChart) {
  const reqId = hyperid();
  logger.debug(`Chart [${reqId}]: ${untrustedChart}`);

  if (typeof untrustedChart === 'string' && untrustedChart.match(LOOPS_REGEX)) {
    return Promise.reject(new Error('Input is not allowed'));
  }

  if (typeof untrustedChart === 'string' && !process.env.FAST_UNSAFE_RENDER) {
    // Fork a process
    return new Promise((resolve, reject) => {
      const proc = getProcess(reqId, resolve, reject);
      proc.send({
        reqId,
        width,
        height,
    ...
    // Safe to do it in this process
    return doRenderChart(width, height, backgroundColor, devicePixelRatio, untrustedChart);
  }
}

async function doRenderChart(width, height, backgroundColor, devicePixelRatio, untrustedChart) {
  let chart;
    ...
    try {
      ...
      const vm = new NodeVM({
        ...
        sandbox: {
          ...
          getGradientFill: (colorOptions, linearGradient = [0, 0, width, 0]) => {
            if (!temporaryCanvas) {
            ...
            colorOptions.forEach((options, idx) => {
              if (typeof options === 'string' && colorOptions.length > 1) {
                gradientFill.addColorStop(idx / (colorOptions.length - 1), options);
              } else {
